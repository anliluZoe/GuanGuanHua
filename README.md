...
- **桌面组件**：每个家庭一张共享照片和一句说明；谁改了，两部手机的小组件一起变。详见 **[docs/widget.md](docs/widget.md)**。
...
5. 桌面组件：打开「我们 → 桌面组件」上传一张合照、写一句说明（建议 ≤40 字），再把「管管花」小组件加到桌面。空状态是「还没有照片」。怎么加、以及 `GET/PATCH /api/widget`、`POST /api/widget/image`：见 **[docs/widget.md](docs/widget.md)**。
